...
namespace WebApis.Net6.Spotify.WebApiEndpoints
{
    public interface IWApiAlbum
    {
        ...
        Task<bool[]?> GetNewReleases(string? country, int limit = 20, int offset = 0, string? accessToken = null);
        Task<Paged<Track>?> GetNextPageAlbumTracks(string nextPage, string? accessToken = null);
        Task<Paged<Album>?> GetNextPageSavedAlbums(string nextPage, string? accessToken = null);
        Task<Paged<Album>?> GetSavedAlbums(int limit = 20, int offset = 0, string? market = null, string? accessToken = null);
