...
namespace SpotifyApi.NetCore
{
    ...
    public class PlayerApi : SpotifyWebApi, IPlayerApi
    {
            ...
            long positionMs = 0)
        {
            ...
            dynamic data = JsonSerializer.Serialize(new { uris = trackIds.Select(SpotifyUriHelper.TrackUri).ToArray() });
        ...
        public async Task PlayAlbum(string albumId, string accessToken = null, string deviceId = null, long positionMs = 0)
        {
            ...
            dynamic data = JsonSerializer.Serialize(new { context_uri = SpotifyUriHelper.AlbumUri(albumId) });
            await Play(data, accessToken, deviceId, positionMs);
        }
        ...
        {
            dynamic data = JsonSerializer.Serialize(new { context_uri = SpotifyUriHelper.AlbumUri(albumId) });
            if (offsetTrackId != null) data.offset = JsonSerializer.Serialize(new { uri = SpotifyUriHelper.TrackUri(offsetTrackId) });
            await Play(data, accessToken, deviceId, positionMs);
        }
        ...
        {
            dynamic data = JsonSerializer.Serialize(new { context_uri = SpotifyUriHelper.AlbumUri(albumId) });
            if (offsetPosition > 0) data.offset = JsonSerializer.Serialize(new { position = offsetPosition });
            await Play(data, accessToken, deviceId, positionMs);
        }
        ...
        public async Task PlayArtist(string artistId, string accessToken = null, string deviceId = null, long positionMs = 0)
        {
            ...
            dynamic data = JsonSerializer.Serialize(new { context_uri = SpotifyUriHelper.ArtistUri(artistId) });
            await Play(data, accessToken, deviceId, positionMs);
        }
            ...
            long positionMs = 0)
        {
            ...
            dynamic data = JsonSerializer.Serialize(new { context_uri = SpotifyUriHelper.PlaylistUri(playlistId) });
            await Play(data, accessToken, deviceId, positionMs);
        }
            ...
            string[] additionalTypes = null)
        {
            ...
            JsonElement? jObject = await GetJObject(builder.Uri, accessToken: accessToken);

            // Todo #25 return 204 no content result 
            if (jObject?.ValueKind == JsonValueKind.Null) return null;

            // Deserialize as Items of Track or Items of Episode
            if (jObject?.EnumerateObject().FirstOrDefault(o => o.Name == "currently_playing_type").Value.ToString() == "episode")
                return jObject?.Deserialize<CurrentEpisodePlaybackContext>();

            return jObject?.Deserialize<CurrentTrackPlaybackContext>();
        }
            ...
            string accessToken = null)
        {
            if (limit.HasValue && (limit.Value < 1 || limit.Value > 50))
                throw new ArgumentOutOfRangeException(nameof(limit), "Limit must be a value from 1 to 50");
