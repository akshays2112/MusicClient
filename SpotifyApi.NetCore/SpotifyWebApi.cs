...
namespace SpotifyApi.NetCore
{
    ...
    public abstract class SpotifyWebApi
    {
        ...
        protected internal virtual async Task<T> GetModelFromProperty<T>(
            ...
            string accessToken = null)
        {
            JsonElement? jObject = await GetJObject(uri, accessToken: accessToken);
            if (jObject is null || jObject?.ValueKind == JsonValueKind.Null) return default;
        ...
        protected internal virtual async Task<JsonElement?> GetJObject(Uri uri, string accessToken = null)
        {
            ...
            if (deserialized.ValueKind == JsonValueKind.Null)
                throw new InvalidOperationException($"Failed to deserialize response as JSON. Response = {json[..Math.Min(json.Length, 256)]}");
        ...
        protected internal virtual async Task<SpotifyResponse<T>> Delete<T>(Uri uri, object data, string accessToken = null)
        {
            ...
            if (response.Content != null)
            {
                ...
                if (!string.IsNullOrEmpty(json)) spotifyResponse.Data = JsonSerializer.Deserialize<T>(json);
            }
        ...
        private async Task<SpotifyResponse<T>> PostOrPut<T>(string verb, Uri uri, object data, string accessToken = null)
        {
            ...
            response = verb switch
            {
                "PUT" => await _http.PutAsync(uri, content),
                "POST" => await _http.PostAsync(uri, content),
                _ => throw new NotSupportedException($"{verb} is not a supported verb"),
            };
